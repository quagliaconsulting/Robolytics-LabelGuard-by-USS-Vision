    ...
    try:
        ...
        for label in labels:
            ...
            if len(parts) == 5:
                ...
                label_bounding_boxes.append({'box': bounding_box, 'class': class_id, 'status': 'FN'})
                ...
                label_bounding_boxes.append({'box': bounding_box, 'class': class_id, 'status': 'FN'})
                logging.debug(f"Added polygon bounding box: {bounding_box}, class: {class_id}")
            else:
        ...
        for box in result.boxes:
            ...
            prediction_bounding_boxes.append({'box': pred_box, 'confidence': confidence, 'class': class_id, 'status': 'FP'})
        ...
        # Update statuses based on TP, FP, FN
        for box in prediction_bounding_boxes:
            if box['status'] == 'FP' and tp > 0:
                box['status'] = 'TP'
                tp -= 1
